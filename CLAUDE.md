...
- **APRS Command** — separate C#/Avalonia desktop APRS client (Jim's). It **replaces YAAC** as FieldCommand's APRS client/feed (YAAC fully removed — see "Radio direction" below). No shared code; data integration only.
...
- **YAAC → APRS Command — FULLY REPLACED (#16, 2026-08).** YAAC is now removed everywhere: `install.sh` no longer installs it (Java/ka2ddo/`/opt/yaac`/`yaac.service`/ufw 8082 all gone), `systemd/yaac.service` deleted, the nginx `/aprs-yc/` (8082) proxy dropped, and `tactical.html` no longer has a YAAC layer/source/WebSocket/poll/send-option/setup-modal. The tactical map's two sources are now **Direwolf (RF) via APRS Command** (port 8080) and **APRS-IS** (internet). All docs (manual ch13, install guides, user guide, BOM, overview, value brief, beta checklist) regenerated without YAAC. APRS Command (separate repo, `C:\Dev\APRS-Command`) has a **FieldCommand LAN feed** mode: its `MobileCompanionServer` can run tokenless on a fixed port (8080) with wildcard CORS, serving the existing `/api/stations` etc. Start it from APRS Command's **View → "FieldCommand Tactical-Map Feed (LAN)"** menu item (`DesktopRuntime.StartFieldCommandFeed`). FieldCommand's `tactical.html` normalizes APRS Command's JSON (`callsign/lat/lng/comment/symbol/lastHeard/speed(knots)/course`). **Deploy wiring (operator, needs hardware):** (1) in APRS Command, connect its TNC to the Pi's Direwolf (KISS TCP `<pi-ip>:8001` or AGW `:8000`); (2) start the LAN feed menu item; (3) on the tactical map, set Settings → APRS Sources → RF source host = laptop IP, port = 8080. Pi-standalone alternative: a tiny KISS→`/api/stations` bridge on localhost:8080. End-to-end (radio→Direwolf→APRS Command→map) is untested pending hardware.
