#!/usr/bin/env bash
# FieldCommand one-command update/maintenance script
set -euo pipefail
...
case "$CHOICE" in
    1)
        ...
        for svc in fcc-lookup health-monitor deadmans ics-platform fieldcommand-refs fieldcommand-tiles direwolf nginx; do
            sudo systemctl restart "$svc" && echo -e "${GREEN}✓ $svc${NC}" || echo -e "${RED}✗ $svc${NC}"
        done
        ;;
    2)
        echo -e "${CYAN}Stopping services...${NC}"
        for svc in fcc-lookup health-monitor deadmans ics-platform fieldcommand-refs fieldcommand-tiles direwolf; do
    ...
    5)
        ...
        sudo -u fieldcommand "$FC_VENV/bin/python" "$FC_HOME/python/build_fcc_db.py"
    ...
    7)
        ...
        ;;
    8)
        DEST="/tmp/fieldcommand-backup-$(date +%Y%m%d_%H%M%S)"
        mkdir -p "$DEST"
    ...
    9)
        ...
        df -h / | tail -1
        du -sh "$FC_HOME/data/" 2>/dev/null && echo "  ↑ fieldcommand data"
        du -sh "$FC_WEB/" 2>/dev/null && echo "  ↑ web files"
        ;;
    a)
        echo -e "${CYAN}Re-downloading ICS forms PDFs...${NC}"
        sudo -u fieldcommand "$FC_VENV/bin/python" "$FC_HOME/python/ics_pdf_downloader.py" \
