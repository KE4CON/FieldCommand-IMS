...
def _qint(qs, key, dflt):
    """Parse an int query param, falling back to a default on bad input."""
    try:
        return int(qs.get(key, [str(dflt)])[0])
    except (ValueError, TypeError, IndexError):
        return dflt


def _safe_insert_parts(d):
    """(cols, placeholders, values) using only valid-identifier column names --
    prevents SQL identifier injection from restored archive JSON."""
    d = {k: v for k, v in d.items() if isinstance(k, str) and k.isidentifier()}
    return ",".join(d.keys()), ",".join("?" * len(d)), list(d.values())
...
class ICSHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path.startswith("/api/ics/incidents/") and not path.endswith(("/archived","/lacie")):
    ...
    def do_POST(self):
        ...
        elif path.startswith("/api/ics/incidents/") and not path.endswith(("/archive","/restore","/delete")):
      ...
      {body_xml}
        ...
        # ── Restore archived incident from USB backup drive ────────────────────────────
            ...
            for inc in data.get("incident",[]):
                ...
                cols, ph, vals = _safe_insert_parts(inc)
                c.execute(f"INSERT OR REPLACE INTO incidents ({cols}) VALUES ({ph})", vals)

            # Restore forms
            for row in data.get("forms",[]):
                cols, ph, vals = _safe_insert_parts(row)
                c.execute(f"INSERT OR REPLACE INTO ics_forms ({cols}) VALUES ({ph})", vals)

            # Restore cost data
            for table, key in [("fema_labor","labor"),("fema_equipment","equipment"),("fema_materials","materials")]:
                for row in data.get(key,[]):
                    cols, ph, vals = _safe_insert_parts(row)
                    c.execute(f"INSERT OR REPLACE INTO {table} ({cols}) VALUES ({ph})", vals)
