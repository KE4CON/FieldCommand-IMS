...
# ── Canvas ────────────────────────────────────────────────────────────────────
# ── Edition transform (ESV vs World) ─────────────────────────────────────
# One source, two editions. ESV fills the "your ..." placeholders with McHenry County
# ESV / MCEMA and keeps the Starcom public-safety branding. World keeps the generic
# "your ..." placeholders and genericizes Starcom to public-safety wording. The literal
# starcom.html page URL is preserved (it is a real filename in the app).
EDITION = 'ESV'
_WORLD_SUBS = [
    ('Starcom Net Logger',      'Public Safety Net Logger'),
    ('STARCOM LOGGER',          'PUBLIC SAFETY LOGGER'),
    ('Starcom Logger',          'Public Safety Logger'),
    ('Starcom Resource Map',    'Public Safety Resource Map'),
    ('RACES/ARES/Starcom',      'RACES/ARES/Public Service'),
    ('RACES / ARES / Starcom',  'RACES / ARES / Public Service'),
    ('RACES, ARES & Starcom',   'RACES, ARES & Public Service'),
    ('ARES  \u00b7  Starcom',    'ARES  \u00b7  Public Service'),
    ('Public safety Starcom nets','Public-safety nets'),
    ('public safety Starcom',   'public-safety'),
    ('Starcom nets',            'public-safety nets'),
    ('Starcom net',             'public-safety net'),
    ('Starcom units',           'public-safety units'),
    ('Starcom personnel',       'public-safety personnel'),
    ('Starcom radio net',       'public-safety radio net'),
    ('Starcom header',          'public-safety header'),
    ('Starcom / Public Safety', 'Public Safety'),
    ('Starcom',                 'public service communications'),
]
def transform(x):
    if not isinstance(x, str):
        return x
    if EDITION == 'ESV':
        x = x.replace('your county', 'McHenry County')
        x = x.replace('your area', 'McHenry County')
        x = x.replace('your emergency management agency',
                      'the McHenry County Emergency Management Agency (MCEMA)')
        x = x.replace('your organization', 'McHenry County ESV')
        return x
    # World edition
    x = x.replace('starcom.html', '\x00U\x00')
    for a, b in _WORLD_SUBS:
        x = x.replace(a, b)
    return x.replace('\x00U\x00', 'starcom.html')


class NC(canvas.Canvas):
    def drawString(self, x, y, s, *a, **k): return super().drawString(x, y, transform(s), *a, **k)
    def drawCentredString(self, x, y, s, *a, **k): return super().drawCentredString(x, y, transform(s), *a, **k)
    def drawRightString(self, x, y, s, *a, **k): return super().drawRightString(x, y, transform(s), *a, **k)
    ...
    def _draw_cover(self):
        ...
        self.drawCentredString(PAGE_W/2, PAGE_H*0.268,
            'Serving your emergency management agency')
        self.setFillColor(HexColor('#6070a0'))
        self.setFont('Helvetica', 9)
        self.drawCentredString(PAGE_W/2, PAGE_H*0.235, TODAY)
        self.setFillColor(HexColor('#1a3a6b'))
...
def P(t, s=None):  return Paragraph(transform(t), s or S('b'))
...
def build_story():
    ...
    story.append(P(
        ...
        'fed by Direwolf / APRS Command, system health indicators, Dead Man Switch state for any active net, '
    ...
    story.append(P('Displays live APRS station positions on an interactive Leaflet map. Pulls data from '
                   'the RF feed (Direwolf via APRS Command) on port 8080, merging it into a unified picture. Works completely offline.'))
    story.append(SP(6))
...
# ── Build both editions ───────────────────────────────────────────────────
_OUTDIR = os.environ.get('FC_USERGUIDE_OUTDIR') or (
...
_UG_EDITIONS = {
    'ESV':   'McHenry_County_RACES_ARES_Starcom_FieldCommand_User_Guide.pdf',
    'World': 'FieldCommand_RACES_ARES_PublicService_User_Guide.pdf',
}
for _edkey, _fname in _UG_EDITIONS.items():
    ...
    doc = SimpleDocTemplate(
        ...
        title=transform('your county RACES/ARES/Starcom FieldCommand User Guide'),
        author='FieldCommand IMS')
    doc.build(build_story(), canvasmaker=NC)
    _pages = None
    try:
        from pypdf import PdfReader, PdfWriter
        for _ap in ['/home/claude/pi500_addendum.pdf']:
            if os.path.exists(_ap):
                w = PdfWriter()
                for p in PdfReader(out).pages: w.add_page(p)
                for p in PdfReader(_ap).pages: w.add_page(p)
                buf = io.BytesIO(); w.write(buf)
                open(out, 'wb').write(buf.getvalue()); break
        _pages = len(PdfReader(out).pages)
    except ModuleNotFoundError:
        pass
    print(f'BUILT [{_edkey}]: {out}' + (f'  ({_pages} pages)' if _pages else ''))
